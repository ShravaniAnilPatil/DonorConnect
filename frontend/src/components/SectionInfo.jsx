import { useNavigate } from "react-router-dom";

const SectionInfo = () => {
  const navigate = useNavigate();

  const handleClick = () => {
    navigate("/find-donors");
  };

  return (
    <section className="bg-gray-100 text-center py-12 px-6">
      <h2 className="text-2xl font-bold mb-4 text-gray-800">WE ARE HERE FOR HELPING PEOPLE</h2>
      <p className="text-gray-600 max-w-2xl mx-auto mb-6">
        You can give blood at any of our blood donation venues. We have several donor centers and visit other venues on various occasions.
      </p>
      <button
        className="bg-red-500 hover:bg-red-600 px-6 py-3 text-white text-lg rounded shadow"
        onClick={handleClick}
      >
        FIND DONOR
      </button>
    </section>
  );
};

export default SectionInfo;

...
const FindDonors = () => {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h2 className="text-3xl font-extrabold mb-8 text-center text-red-700 drop-shadow-sm">
        Find Donors
      </h2>
      ...
      <div className="mb-8 flex justify-center">
        <select
          value={selectedGroup}
          onChange={handleChange}
          className="p-3 border border-gray-300 rounded-lg shadow-sm text-gray-700 focus:ring-2 focus:ring-red-400 focus:outline-none"
        >
          <option value="">All Blood Groups</option>
          {["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"].map((group) => (
            <option key={group} value={group}>
      ...
      <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-6">
        {donors.length > 0 ? (
          donors.map((donor, index) => (
            <div
              key={index}
              className="p-6 border rounded-xl bg-white shadow-md hover:shadow-lg transition duration-200"
            >
              <h3 className="text-xl font-bold text-red-700 mb-2">{donor.name}</h3>
              ...
              <button
                className="mt-4 px-4 py-2 bg-red-600 text-white font-medium rounded-lg shadow hover:bg-red-700 active:scale-95 transition"
                onClick={() => toggleForm(index, donor.blood_group)}
              >
                Request Blood
              </button>
              ...
              {showFormIndex === index && (
                <form
                  className="mt-5 space-y-3 bg-gray-50 p-4 rounded-lg border"
                ...
                >
                  ...
                  <button
                    type="submit"
                    className="w-full bg-green-600 text-white py-2 rounded-lg shadow hover:bg-green-700 active:scale-95 transition"
                  >
                    Submit Request
                  </button>
                </form>
